...
export default function FrequentlyAskedQuestions() {
    ...
    return (
        <div className="flex flex-col w-full min-h-[800px] items-center justify-start py-[6rem] gap-[4rem] bg-[#FAFAFA]">
            <span className="text-[48px] text-[#1D242B] font-[900]">FREQUENTLY ASKED QUESTIONS</span>

            <div className="flex flex-col items-center w-[1200px] justify-center">
                {FrequentQuestions.map((faq) => (
                    <div key={faq.id} className="flex flex-col items-center justify-start w-full h-auto border-b border-b-[#1D242B]">
                        <div className="flex items-center justify-between w-full py-[0.5rem]">
                            <span className="flex items-center text-[24px] font-bold">{faq.question}</span>
                        ...
                        {questionOpen === faq.id && (
                            <div className={`flex flex-col items-start w-full py-[1rem] gap-2`}>
                                {typeof faq.answer === "string" ? (
                                    <span className="text-[20px] text-[#1D242B] leading-[1]">
                                        {faq.answer}
                                    </span>
                                ) : (
                                    faq.answer.map((ans, index) => (
                                        <div key={index} className="flex flex-col items-start justify-start w-full">
                                            <div className="flex items-start w-full">
                                                <span className="text-[20px] text-[#1D242B] pr-2 font-bold">{`${String.fromCharCode(96 + (index + 1))}.`}</span>
                                                <span className="text-[20px] text-[#1D242B]">
                                                    {ans}
                                                </span>
                                            </div>
                                        </div>
