...
export default function InquiryFormClient ({ 
    ...
    const [payload, setPayload] = useState<InquiryFormValues[]>([]);
    ...
    const onSubmit = async (data: InquiryFormValues) => {
        try {
            // await SubmitInquiry(data)  >>> UNCOMMENT WHEN GoHighLevel form submission is ready
                ...
                !isSubmitSuccessful ? (
                    ...
                    <div className='flex flex-col items-center bg-[#C7EEFF] gap-2 p-4 py-5 w-full'>
                        {payload.map((payload, index) => (
                            <div key={index} className='flex flex-col items-center w-full bg-[#FAFAFA] p-4 gap-2 rounded-[15px]'>
