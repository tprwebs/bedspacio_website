...
export default function DashboardPageWrapper ({
    ...
    return (
        <div className="flex w-full h-screen">
            ...
            <div className="flex flex-col w-full p-[1rem] xl:px-[8rem] lg:px-[1rem] gap-[1rem]">

                <div className='flex items-center w-full justify-between py-[1rem]'>
                    <span className='text-[18px] font-bold'>Hello, Angelo Cabangal!</span>
                ...
                <div className='flex flex-col w-full rounded-[5px] gap-2'>
                    ...
                    <div className="grid grid-cols-4 w-full rounded-[5px] gap-2">
                        ...
                        {inquiries.map((inq, index) => (
                            <Link key={index} href={`/admin/inquiry?status=${inq.status}`} className="flex flex-col w-full gap-2 items-start justify-evenly p-4 rounded-[10px] cursor-pointer border border-[#1D242B]/50 hover:bg-[#0077C0]/10 hover:-translate-y-1 transition-all duration-100">
                                <div className="flex items-center justify-start gap-2 w-full">
                                    <Time className="w-[25px] h-[25px] opacity-75" />
                                    <span className="text-[14px] font-bold text-[#1D242B] opacity-80">{inq.status}</span>
                                </div>
                ...
                <div className='grid grid-cols-[3fr_2fr] h-[500px] w-full gap-[1rem] '>

                    <div className='flex flex-col border border-[#1D242B]/50 p-4 rounded-[10px] gap-[1rem]'>
                        <div className='flex items-center justify-between w-full'>
                            <div className='flex items-center gap-2'>
                                <span className='text-[22px] text-[#1D242B]/90 font-bold'>Recent Inquiries</span>
                                <Arrow className="w-[25px] h-[25px]" />
                            </div>

                            <Link href={'/admin/inquiry'} className='flex items-center gap-1 opacity-80 hover:opacity-100 active:translate-x-1 transition-all duration-100'>
                                ...
                                {recentInquiries.length > 0 ? (
                                    recentInquiries.map(recent => (
                                        <div key={recent.id} className='grid grid-cols-[10%_20%_20%_20%_15%_15%] place-items-center justify-items-center w-full text-[14px] text-[#1D242B] border border-[#1D242B]/20 py-2 font-bold rounded-[5px] bg-[#C7EEFF]/50'>
                                            ...
                                            <button className='cursor-pointer flex items-center gap-1 bg-[#0077C0]/50 hover:bg-[#0077C0]/75 active:bg-[#0077C0]/50 rounded-full px-2 py-1 text-[12px]'>
                                                <span>View</span>
